...
namespace ROHC {
	...
	output.reserve(data.size());
        ...
        contexts_t::iterator ctx = find_if(contexts.begin(), contexts.end(), ContextFinder(profileId, ip));
        
        if (contexts.end() == ctx)
        {
            // TODO find first null context
            for (ctx = contexts.begin(); contexts.end() != ctx; ++ctx)
            {
                if (!*ctx)
                    break;
            }
            ...
            else
            {
                if (contexts.size() <= maxCID) {
                   ...
                   OldestContextFinder finder; 
                    ctx = min_element(contexts.begin(), contexts.end(), finder);
                    cid = (*ctx)->CID();
                    delete *ctx;
                }
            }
            
            profile = CProfile::Create(this, cid, profileId, ip);
